...
export default function Menu() {
  ...
  return (
    <section id="menu" className="relative border-t border-cream-100/10 py-24 sm:py-32">
      <div className="mx-auto max-w-6xl px-6 sm:px-10">
        <div ref={headRef} className="reveal flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="font-body text-xs font-bold uppercase tracking-[0.3em] text-flame-500">{menu.eyebrow}</p>
            <h2 className="mt-5 font-display text-4xl uppercase text-cream-100 sm:text-5xl">{menu.heading}</h2>
            <p className="mt-4 max-w-md font-body text-sm text-cream-200/70">{menu.subheading}</p>
          </div>
        ...
        >
          {category.items.map((item) => (
            ...
            >
              {/* image slot — swap `item.image` in content.js for real food photos */}
              ...
              <div className="p-6">
                <h3 className="font-display text-xl uppercase text-cream-100">{item.name}</h3>
                <p className="mt-2 font-body text-sm leading-relaxed text-cream-200/70">{item.note}</p>
              </div>
