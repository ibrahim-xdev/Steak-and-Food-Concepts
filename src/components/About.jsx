    ...
    >
      <div className="mx-auto grid max-w-6xl gap-16 px-6 sm:px-10 lg:grid-cols-[1.1fr_0.9fr] lg:gap-12">
        <div ref={textRef} className="reveal">
          ...
          <div
            ref={statsRef}
            className="reveal-stagger mt-12 grid grid-cols-3 gap-6 border-t border-cream-100/10 pt-8"
          >
            {about.stats.map((s) => (
              <div key={s.label}>
                <div className="font-display text-2xl text-mustard-400 sm:text-3xl">
                  {s.value}
                </div>
                <div className="mt-2 font-body text-xs leading-snug text-cream-200/70">
                  {s.label}
                </div>
              </div>
            ))}
        ...
        <div ref={cardRef} className="reveal lg:pt-4">
          <div className="relative overflow-hidden rounded-2xl border border-cream-100/10 bg-char-800/70 p-8">
            ...
            <div className="relative flex items-baseline gap-2">
              <span className="font-display text-6xl text-mustard-400">
            ...
            <dl className="relative mt-8 space-y-4 border-t border-cream-100/10 pt-6 font-body text-sm">
              <div className="flex items-center justify-between gap-4">
                ...
                <dd className="text-right font-semibold text-cream-100">
                  {shop.priceRange}
                </dd>
              </div>
              <div className="flex items-center justify-between gap-4">
                <dt className="text-cream-200/60">Service</dt>
                <dd className="text-right text-cream-100">
                  {shop.services.join(" · ")}
                </dd>
              </div>
              <div className="flex items-center justify-between gap-4">
                ...
                <dd className="text-right text-cream-100">
                  11:30 AM - 2:30 PM, 5:30 PM - 9:00 PM (Tuesday - Saturday)
                </dd>
              </div>
            </dl>
          </div>
...
function Star({ filled }) {
  return (
    <svg
      viewBox="0 0 20 20"
      className={`h-4 w-4 ${filled ? "fill-mustard-400" : "fill-cream-100/15"}`}
    >
